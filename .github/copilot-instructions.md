...
Controller Validation: Use @Valid in the controller method arguments

E. Documentation

Always use Javadoc comments for public methods in the Service layer

Always use Swagger/OpenAPI annotations on your Controller methods.

2. Frontend (React.js + TypeScript) - Ant Design System
A. API Client (Axios)
...
C. Tools
- Ant Design: Use Ant Design components for UI consistency. Customize the theme as needed.

- Zustand: Use Zustand for global state management. Keep the store logic separate from UI components.
...
- Do not use `any`
- Use TypeScript interfaces/types for all props and API responses. Write to `models`